...
def transform(input_path: str, output_path: str, multiplier: int = 10,
    ...
    # FIX: mdhd/tkhd/mvhd for the video track were left at the ORIGINAL
    # duration while stts now declares `total_frames` samples -- an
    # internally inconsistent file per ISO/IEC 14496-12 (sum(stts) must
    # equal mdhd.duration). jitter_durations() already preserves this
    # sum (see its docstring), so compute it once here, pre-jitter, and
    # reuse it below both for the header fields and for the jitter step.
    video_deltas_raw = expand_stts(video_stts) + [time_delta] * pad_count
    new_v_duration = sum(video_deltas_raw)                      # ticks, video mdhd timescale
    new_v_duration_ms = new_v_duration * 1000 // v_timescale    # ms, movie/tkhd timescale
    ...
    new_mvhd_dur = max(new_v_duration_ms, new_audio_dur_ms) if has_audio else new_v_duration_ms
    mvhd = build_mvhd(mvhd_content, new_mvhd_dur)

    # Video track components
    video_tkhd = build_tkhd(data[vt_start:vt_start + read_u32(data, vt_start)],
                             duration_ms=new_v_duration_ms)
    edts_pos, edts_size = find_box(data, 'edts', vt_start, vt_end)
    video_edts = data[edts_pos:edts_pos + edts_size] if edts_pos else b''
    video_mdhd = build_mdhd(data[v_mdhd_pos + 8:v_mdhd_pos + v_mdhd_size],
                             duration=new_v_duration)
    ...
    video_deltas = jitter_durations(video_deltas_raw, rand)
